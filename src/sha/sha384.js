...
*/
const sha384 = (data, salt=null, encoding='hex') => {
	if (Buffer.isBuffer(data)) data.toString(encoding);
	return crypto.createHash('sha384').update(mix(data, salt)).digest().toString(encoding);
};
...
*/
const sha384rsa = (data, publickey=null, privatekey=null, salt=null, encoding='hex') => {
	...
	return {
		hash: crypto.publicEncrypt({key: publickey},sha384(data, salt, encoding)).toString(encoding),
		publickey,
		privatekey
	};
};
